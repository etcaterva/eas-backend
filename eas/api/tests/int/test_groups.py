...
from eas.api.models import Groups

from ..factories import GroupsFactory
from .common import DrawAPITestMixin


class TestGroups(DrawAPITestMixin, APILiveServerTestCase):
    maxDiff = None
    base_url = "groups"
    Model = Groups
    ...
    def _transform_draw(self, draw, write_access):
        return {
            **super()._transform_draw(draw, write_access),
            "number_of_groups": draw.number_of_groups,
            "participants": [
                dict(
                    id=p.id,
    ...
    def test_insuficient_participants2(self):
        draw = self.Factory(number_of_groups=10)
        url = reverse(f"{self.base_url}-toss", kwargs=dict(pk=draw.private_id))
        response = self.client.post(url, {})
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.content
